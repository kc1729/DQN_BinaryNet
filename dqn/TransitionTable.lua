...
-- fills the experience buffer. is called by sample. it calls sample_one.
-- basically sample one samples one random non-terminal state, and 
-- fill buffer stores bufferSize number of such states, ac, re,ter, s2.
...
-- concatFrames concatenates 4 consecutivev frames as a state
...
-- inner for loop runs only once becausein our case, histSpacin = 1
-- so histIndices values are consecutive
